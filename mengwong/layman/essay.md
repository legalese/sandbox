...
The worked example is real and runs. The statute-as-model is `cheating-415-poh-yuan-nie.l4`; the surplusage proof is `cheating-415-surplusage.z3.py`; the minimiser that deletes the dead clause is `cheating-415-espresso.py`; the ladder figure is drawn by `cheating-415-ladder.py`. The case is *Poh Yuan Nie v Public Prosecutor* [2022] SGCA 74; the bracketed numbers are the court's own paragraphs.
